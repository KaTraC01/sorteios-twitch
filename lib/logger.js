 ...
 * SEGURANÇA: Logs sanitizados para remover informações sensíveis
 */

const { sanitizeLogMessage, sanitizeLogObject } = require('../src/utils/logSanitizer');
...
function info(mensagem, ...args) {
  const sanitizedMessage = sanitizeLogMessage(mensagem);
  const sanitizedArgs = args.map(arg => sanitizeLogObject(arg));
  
  if (!isProduction || shouldShowDetailedLogs()) {
    const timestamp = new Date().toISOString();
    console.log(`[INFO][${timestamp}] ${sanitizedMessage}`, ...sanitizedArgs);
  } else {
